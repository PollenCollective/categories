...
impl<O: Object, P: PowerObjectGenerator<O>> Category<O, P> {
    ...
    pub fn create(generator: Option<P>) -> Self {
        let power_objects = if generator.is_none() {
            None
        } else {
            Some(HashMap::new())
        };
    ...
    pub fn add_morphism(
        ...
        if self.objects.get(domain) != Some(actual_domain) {
            return Err(format!(
                "Domain index {domain} does not match morphism's domain object"
            ));
        }
        if self.objects.get(codomain) != Some(actual_codomain) {
            return Err(format!(
                "Codomain index {codomain} does not match morphism's codomain object"
            ));
        }
    ...
    pub fn fetch_power_object_id(&mut self, power_type: PowerObjectType) -> Result<usize, String> {
        ...
        let (new_obj, new_morphisms) = self
            .generator
            .as_ref()
            .unwrap()
...
impl<O: Object, P: PowerObjectGenerator<O>> CategoryBuilder<O, P> {
    ...
    pub fn build(self) -> Category<O, P> {
        ...
        if self.pobject_generator.is_some() {
            let genr = self.pobject_generator.as_ref().unwrap();
            for (i, _) in self.objects.iter().enumerate() {
                for (j, _) in self.objects.iter().enumerate() {
                    ...
                    for var in variants {
                        let (obj, homset) = genr.generate_power_object(&var, &objects);
                        for morph in homset {
                            let dom = morph.domain();
                            let cod = morph.codomain();
                            let (sidx, _) =
                                objects.iter().enumerate().find(|&(_, x)| x == dom).unwrap();
                            let (eidx, _) =
                                objects.iter().enumerate().find(|&(_, x)| x == cod).unwrap();
...
mod tests {
    ...
    impl Morphism for IdMorphism {
        ...
        fn map(&self, d: &Self::Domain) -> Self::Codomain {
            d.clone()
        }
    }
    ...
    impl Morphism for ConstMorphism {
        ...
        fn map(&self, _d: &Self::Domain) -> Self::Codomain {
            self.codomain.clone()
        }
    }
    ...
    impl PowerObjectGenerator<SetObj> for NoGen {
        fn generate_power_object(
            ...
            SetObj,
            Vec<Box<dyn Morphism<Domain = SetObj, Codomain = SetObj>>>,
        ) {
    ...
    fn compose_constant_chain() {
        ...
        let f: Box<dyn Morphism<Domain = _, Codomain = _>> = Box::new(ConstMorphism {
            domain: a.clone(),
            codomain: b.clone(),
        });
        let g: Box<dyn Morphism<Domain = _, Codomain = _>> = Box::new(ConstMorphism {
            domain: b.clone(),
            codomain: c.clone(),
        });
    ...
    fn add_morphism_deduplicates() {
        ...
        let dup: Box<dyn Morphism<Domain = _, Codomain = _>> = Box::new(ConstMorphism {
            domain: a.clone(),
            codomain: b.clone(),
        });
    ...
    fn identity_is_monic() {
        ...
        let f1: Box<dyn Morphism<Domain = _, Codomain = _>> = Box::new(ConstMorphism {
            domain: a.clone(),
            codomain: b.clone(),
        });
        let f2: Box<dyn Morphism<Domain = _, Codomain = _>> = Box::new(ConstMorphism {
            domain: a.clone(),
            codomain: b.clone(), /* same codomain */
        });
        ...
        assert!(cat
            .is_monic(1, 1, 0 /* idx of id_B in hom-set (1,1) */)
            .unwrap());
    }
